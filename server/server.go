...
func InitServer(ctx context.Context, fakeClientNumber *int, debugMainRoom bool) {
	...
	if turnEnabled || *fakeClientNumber > 0 {
	...
	roomManager = sfu.NewManager(ctx, "gocall-manager", sfuOpts)
	defaultRoom, _ := CreateRoom("admin", "main", debugMainRoom)
	createFakeClient(ctx, *fakeClientNumber, defaultRoom, roomManager)
}

func HandleSFU(conn *websocket.Conn) {
	messageChan := make(chan Request)
	defer close(messageChan)

	// Читаем входящие сообщения в отдельной горутине
	go reader(conn, messageChan)
	clientHandler(false, conn, messageChan, nil, roomManager)
}
...
func GetRoomManager() *sfu.Manager {
	return roomManager
}

// create room and sotre its name in db
func CreateRoom(owner string, roomName string, isDebug bool) (*sfu.Room, error) {
	db, _ := database.GetDatabase()
	defer db.Close()
	roomID := roomManager.CreateRoomID()
	err := database.RegisterRoom(db, roomID, owner)
	if err != nil {
		return nil, fmt.Errorf("failed to register room: %v", err)
	}
	// create new room
	...
	room, err := roomManager.NewRoom(roomID, roomName, sfu.RoomTypeLocal, roomsOpts)
	if err != nil {
		return nil, fmt.Errorf("failed to create room: %v", err)
	}
	go func() {
		log.Printf("Starting handler for room: %s", roomName)
		for {
			select {
			case <-room.Context().Done():
				log.Printf("Stopping handler for room: %s", roomName)
				return
			// Здесь можно добавить обработку событий комнаты, если нужно
			}
		}
	}()
	return room, nil
...
// 
func createFakeClient(ctx context.Context, fakeClientCount int, room *sfu.Room, roomManager *sfu.Manager) {
	...
	for i := 0; i < fakeClientCount; i++ {
		...
		fc := fakeclient.Create(ctx, roomManager.Log(), room, iceServers, fmt.Sprintf("fake-client-%d", i), true)
...
func clientHandler(isDebug bool, conn *websocket.Conn, messageChan chan Request, r *sfu.Room, roomManager *sfu.Manager) {
	ctx, cancel := context.WithCancel(conn.Request().Context())
	defer cancel()
